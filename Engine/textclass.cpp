...
bool TextClass::SetFinishedText(ID3D11DeviceContext* deviceContext) {
	char dataString[16];
	bool result;

	// Setup the video card name string.
	strcpy_s(dataString, "Press F!");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, dataString, 400, 500, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
